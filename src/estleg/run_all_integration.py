...
STEPS: list[dict] = [
    ...
    {
        "name": "rebuild_eurlex_combined",
        "description": "Rebuild eurlex_combined.jsonld from peeps after transposition (#417)",
...
def _run_logged(cmd: list[str], log_path: Path, timeout: int, banner: str) -> int:
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        log_file.write(f"# {banner} {datetime.now(UTC).isoformat()}\n")
        log_file.flush()
        try:
