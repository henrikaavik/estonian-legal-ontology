...
def discover_repo_root() -> Path:
    """Locate the checkout that owns this package (issue #472)."""
    start = Path(__file__).resolve().parent
    for candidate in [start, *start.parents]:
        if (candidate / "pyproject.toml").is_file() and (
            candidate / "estleg_client"
        ).is_dir():
            return candidate
...
# ``estleg:totalFiles`` / ``estleg:fileCount`` (currently 27228), which
...
# count (currently 27228) is unchanged: the only ``*.json`` currently living under
